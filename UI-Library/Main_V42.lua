--[[
    ...
    Github: https://github.com/Footagesus/FurBox
...
local B=v.Object:FindFirstChild"FurBoxGradient"
if B then
B:Destroy()
end
...
local M="FurBox/"..C.."/assets/."..F.."-"..x..".png"
...
warn(string.format("[ FurBox.Creator ] Failed to load custom asset '%s': %s",M,tostring(P)))
L:Destroy()
...
if not N then
warn("[ FurBox.Creator ]  '"..identifyexecutor().."' doesnt support the URL Images. Error: "..O)
...
return[[
{
    ...
    "version": "1.6.62",
    "main": "./dist/main.lua",
    "repository": "https://github.com/Footagesus/FurBox",
    "discord": "https://discord.gg/ftgs-development-hub-1300692552005189632",
    "author": "Footagesus",
...
local ak=a.load'l'.Init(nil,ag.FurBox.ScreenGui.KeySystem)
...
local g=ag.FurBox.Services[f.Type]
if g then
local h={}
...
al:Close()()
writefile(("FurBox").."/"..ah..".key",tostring(aA))
task.wait(.4)
ai(true)
end
...
local ag=a.load'l'.Init(nil,ae.FurBox.ScreenGui.Popups)
...
warn"[ FurBox.ConfigManager ] Window.Folder is not specified."
return false
end

ac=af
ad.Folder=ac.Folder
ad.Path="FurBox/"..tostring(ad.Folder).."/config/"

if not isfolder("FurBox/"..ad.Folder)then
makefolder("FurBox/"..ad.Folder)
if not isfolder("FurBox/"..ad.Folder.."/config/")then
makefolder("FurBox/"..ad.Folder.."/config/")
end
end
...
warn"[ FurBox.ConfigManager ] The config system doesn't work in the studio."
return nil
end
...
if ak then
if ac.Debug then print("[ FurBox.ConfigManager ] AutoLoaded config: "..af)end
else
warn("[ FurBox.ConfigManager ] Failed to AutoLoad config: "..af.." - "..tostring(al))
end
end)
end
end
...
aq.Load(al,ap.Content,aq.Elements,ak.Window,ak.FurBox,function()
...
warn("[ FurBox ] Failed to apply pending config for '"..ar.Flag.."': "..tostring(ay))
end
end)
end
end
...
az.Load(ap,ap.UIElements.ContainerFrame,az.Elements,Window,FurBox,nil,az,ao)
...
if as.Folder then
if not isfolder("FurBox/"..as.Folder)then
makefolder("FurBox/"..as.Folder)
end
if not isfolder("FurBox/"..as.Folder.."/assets")then
makefolder("FurBox/"..as.Folder.."/assets")
end
...
if not f then
warn("[ FurBox.Window.Background ] Failed to download video: "..tostring(g))
return
end
end
...
if not f then
warn("[ FurBox.Window.Background ] Failed to load custom asset: "..tostring(g))
return
end
warn"[ FurBox.Window.Background ] VideoFrame may not work with custom video"
aF=g
end
...
local j=0
local l=as.UIElements.Main.Main.Topbar.Right.UIListLayout.AbsoluteContentSize.X/ar.FurBox.UIScale





j=as.UIElements.Main.Main.Topbar.Left.AbsoluteSize.X/ar.FurBox.UIScale
...
0,
j+(as.UIPadding/ar.FurBox.UIScale),
0.5,
0
)
as.UIElements.Main.Main.Topbar.Center.Size=UDim2.new(
1,
-j-l-((as.UIPadding*2)/ar.FurBox.UIScale),
1,
0
)
end)
...
as.UIElements.Main.Main.Topbar.Left.Position=UDim2.new(0,(as.UIElements.Main.Main.Topbar.Right.AbsoluteSize.X/ar.FurBox.UIScale)+as.UIPadding-4,0,0)
end)
end
...
ImageTransparency=as.Transparent and ar.FurBox.TransparencyValue or 0
},{
l
})
end
...
as.Destroyed=true
task.wait(0.4)
ar.FurBox.ScreenGui:Destroy()
ar.FurBox.NotificationGui:Destroy()
ar.FurBox.DropdownGui:Destroy()
ar.FurBox.TooltipGui:Destroy()
...
as.Transparent=r
ar.FurBox.Transparent=r

as.UIElements.Main.Background.ImageTransparency=r and ar.FurBox.TransparencyValue or 0
...
return ar.FurBox.UIScale
end

function as.SetUIScale(p,r)
ar.FurBox.UIScale=r
al(ar.FurBox.UIScaleObj,.2,{Scale=r},Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
return as
end
...
local p=a.load'U'
local r=a.load'V'
local u=p.Init(as,ar.FurBox,ar.FurBox.TooltipGui)
...
return u.New(x,ar.FurBox.UIScale)
end
...
return r.New(x,as.UIElements.SideBar.Frame,as.Folder,ar.FurBox.UIScale,as)
end
...
local R=J.AbsoluteContentSize.X/ar.FurBox.UIScale
local S=L.AbsoluteSize.X/ar.FurBox.UIScale
...
local Y=X.AbsoluteSize.X/ar.FurBox.UIScale
if Y<V then
V=Y
U=X
end
...
Name="FurBox/Notifications",
Parent=au,
IgnoreGuiInset=true,
})
aa.DropdownGui=ap("ScreenGui",{
Name="FurBox/Dropdowns",
Parent=au,
IgnoreGuiInset=true,
})
aa.TooltipGui=ap("ScreenGui",{
Name="FurBox/Tooltips",
Parent=au,
IgnoreGuiInset=true,
})
as(aa.ScreenGui)
